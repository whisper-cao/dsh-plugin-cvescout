...
export function registerAtomicTools(ctx, runtime) {
    ...
    ctx.tools.register(defineTool({
        name: 'fingerprint',
        description: '识别目标技术栈：服务器、框架、CMS、前端库、WAF、前置代理，并采集 Allow 方法、错误页特征与**协议能力**（TLS 版本、ALPN/HTTP-2 是否可用、证书事实）。全部为被动探测：HTTP 侧只用 GET/HEAD/OPTIONS 与固定被动路径，协议侧只做一次裸 TLS 握手、不发任何载荷，不做任何写入。当需要「先摸清某个站点用什么组件、是否开着 HTTP/2」而不针对特定 CVE 时用本工具。',
        parameters: {
            url: { type: 'string', required: true, description: TARGET_PARAM_DESCRIPTION },
