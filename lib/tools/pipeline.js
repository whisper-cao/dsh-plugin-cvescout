...
import { normalizeCveId } from "../sources/nvd.js";
import { retestBatch, retestOne } from "../core/retest.js";
import { executeRepro, parseReproDoc, sanitizePayload } from "../core/pentest.js";
import { asJsonValue, fromJsonValue } from "./json.js";
...
const TARGET_PARAM_DESCRIPTION = '目标地址：完整 URL、裸域名（portal.example.com）、host:port 均可；未写协议时按 defaultScheme 补齐，连接失败自动换协议重试一次；也接受 cordis.yml 中 targetAliases 配置的系统别名';
/** 从参数里拆分 CVE 编号列表（支持逗号、顿号、分号、空白）。 */
function splitCveIds(raw) {
...
function renderWithHeadline(headline, value) {
    return [{ type: 'text', text: `${headline}\n\n${JSON.stringify(value, null, 2)}` }];
}
/** 从参数里拿到复现文档内容（内联内容优先，其次读文件）。 */
async function resolveDocument(document, documentPath) {
...
/** 注册编排级工具。 */
export function registerPipelineTools(ctx, runtime) {
    // ------------------------------------------------------- 单 CVE 复测
    ctx.tools.register(defineTool({
        name: 'cve_retest',
        description: '【CVE 复测主入口】当用户说「某网站/系统涉及某个 CVE，帮我排查/验证/确认是否受影响/复测一下」时，优先用本工具。它会自动完成：取或复用目标指纹情报 → 拉取 CVE 受影响组件与版本区间 → 静态判读 → 必要时被动探测取证 → 输出 VULNERABLE / NOT_VULNERABLE / UNCERTAIN、置信度、依据与限制说明。' +
        ...
        output: {
            schema: { type: 'json' },
            render: (_args, value) => {
                const result = fromJsonValue(value);
                if (result.total !== undefined) {
                    return renderWithHeadline(`批量复测完成：共 ${result.total} 个 CVE`, value);
                }
                return renderWithHeadline(`[${result.cveId}] ${result.verdict}（置信度 ${result.confidence}）— ${result.reason}`, value);
            },
        },
        async execute(args, exec) {
            ...
            if (cveIds.length === 0)
                throw new Error('请提供至少一个合法 CVE 编号');
            ...
            if (cveIds.length > 1) {
                return asJsonValue(await retestBatch(runtime, target, cveIds, { forceRecon: args.force_recon === true, reproDoc }, exec.signal));
            }
            return asJsonValue(await retestOne(runtime, target, cveIds[0], { forceRecon: args.force_recon === true, reproDoc }, exec.signal));
        },
    }));
    // ------------------------------------------------------- 批量复测
    ctx.tools.register(defineTool({
        name: 'cve_batch_retest',
        description: '对同一目标的多个 CVE 做批量复测，复用一次指纹侦察，最后给出统计与 Markdown 汇总报告。当用户一次列出多个「站点 + CVE」组合且目标是同一个时用本工具；报告需要归档时把 save_report 置 true。',
        parameters: {
            url: { type: 'string', required: true, description: TARGET_PARAM_DESCRIPTION },
            cve_ids: {
                type: 'string',
        ...
        output: {
            schema: { type: 'json' },
            render: (_args, value) => {
                const batch = fromJsonValue(value);
                return renderWithHeadline(`批量复测完成：共 ${batch.total} 个 CVE，存在漏洞 ${batch.stats.vulnerable}、不受影响 ${batch.stats.notVulnerable}、需人工确认 ${batch.stats.uncertain}`, value);
            },
        },
        async execute(args, exec) {
            const cveIds = args.cve_ids
                ...
                .filter(Boolean);
            if (cveIds.length === 0)
                throw new Error('cve_ids 为空，至少需要一个 CVE 编号');
            const target = resolveTarget(runtime, args.url);
            ...
            return asJsonValue(await retestBatch(runtime, target, cveIds, {
    ...
    ctx.tools.register(defineTool({
        ...
        description: '解析复现文档（Markdown 或 JSON），输出结构化步骤、每步的 HTTP 方法与指标，并标出哪些步骤属破坏性（执行时会被跳过）。本工具不发任何网络请求。',
        parameters: {
    ...
    ctx.tools.register(defineTool({
        name: 'pentest_repro',
        description: '对目标执行复现文档中的非破坏性步骤（破坏性步骤自动跳过，载荷先做无害化替换）。dry_run=true 时只做解析与载荷安全化预览，不发请求。全部请求受安全护栏与时序限制约束。' +
        ...
        output: {
            schema: { type: 'json' },
            render: (_args, value) => {
                const report = fromJsonValue(value);
                return renderWithHeadline(`[${report.cveId}] 非破坏性复现${report.overallSuccess ? '取得正向证据' : '未取得正向证据'}（${report.stepsSuccess}/${report.stepsTotal} 步成功）`, value);
            },
        },
        async execute(args, exec) {
            const target = resolveTarget(runtime, args.url);
            ...
            if (!content)
                throw new Error('需要提供 document 或 document_path 之一');
