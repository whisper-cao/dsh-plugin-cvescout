...
import { emptyTlsIntel, probeTls } from "../core/tls.js";
/** 指纹规则表。 */
...
export async function fingerprintTarget(runtime, targetUrl, signal) {
    ...
    try {
        ...
        if (error instanceof SafetyBlockedError && error.isScopeViolation)
            throw error;
    ...
    // 就把它降级成一条 probeError。
    const scopeViolation = failures.find((item) => item instanceof SafetyBlockedError && item.isScopeViolation);
    if (scopeViolation)
 ...
 */
function pickBaseUrl(original, finalUrl, probeErrors) {
    ...
    if (originalHost === finalHost)
        return finalUrl;
    probeErrors.push(`根路径重定向到 ${finalUrl}（跨主机，后续探测仍针对原目标 ${original}）`);
    return original;
}
/** 抽取 HTTP 层协议线索。 */
export function extractProtocolHints(headers) {
