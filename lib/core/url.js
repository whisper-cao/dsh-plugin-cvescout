...
function cleanup(raw) {
    ...
    if (embedded)
        return embedded[0].replace(TRIM_CHARS, '');
    return text.replace(TRIM_CHARS, '');
}
function lookupAlias(input, aliases) {
    if (!aliases)
        return null;
...
/** 输入里是否已经写明协议。 */
export function hasExplicitScheme(raw) {
 ...
 */
export function resolveTargetInput(raw, options = {}) {
    const cleaned = cleanup(raw);
    if (!cleaned)
        throw new HttpRequestError('invalid-url', '目标为空，请给出域名或 URL');
...
export function hostOf(raw, defaultScheme) {
    const candidates = [raw];
    if (!hasExplicitScheme(raw))
        candidates.push(`${defaultScheme ?? 'https'}://${cleanup(raw)}`);
