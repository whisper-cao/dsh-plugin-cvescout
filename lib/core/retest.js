...
import { normalizeKey, resolveReportDir } from "./cache.js";
import { request } from "./http.js";
import { assessHttp2Precondition, formatRange } from "./judge.js";
import { executeRepro } from "./pentest.js";
import { fingerprintTarget } from "../sources/fingerprint.js";
import { lookupCve, normalizeCveId } from "../sources/nvd.js";
...
/** 把指纹结果转成情报模型。 */
export function intelFromFingerprint(targetUrl, fingerprint) {
    return {
        targetUrl,
...
export async function ensureIntel(runtime, targetUrl, forceRecon, signal) {
    if (!forceRecon) {
        const cached = await runtime.cache.get(normalizeKey(targetUrl));
        if (cached)
            return { intel: cached, cacheHit: true, fresh: null };
    }
...
export async function collectEvidence(runtime, targetUrl, fresh, intel, signal) {
    if (fresh) {
        return [
            {
                url: targetUrl,
        ...
        return [
            {
                ...
                allowedMethods: intel.allowedMethods,
                interestingHeaders: {},
                note: `存活确认失败: ${error.message}`,
            },
        ];
    }
}
function pickInteresting(headers) {
    const picked = {};
...
/** 单个 CVE 复测。 */
export async function retestOne(runtime, targetUrl, cveIdRaw, options = {}, signal) {
    ...
    if (cve.error) {
        ...
        if (judgment.exclusionBasis === 'protocol') {
            // 协议前置条件被实测证伪：依据来自一次真实握手，不依赖版本号是否可见，
            // 因此即便目标隐藏了版本（如 Server: nginx）也能给出确定结论。
            confidence = 0.9;
            limitations.push('结论基于协议前置条件实测排除（不经版本比对）；若被测入口与实际承载业务的入口不是同一处，需人工确认');
        }
        else {
            confidence = 0.95;
        ...
        if (judgment.applicable === 'yes') {
            ...
            reason = `${judgment.reason}；${poc.pocCount > 0
    ...
    // 依赖协议前置条件的 CVE：确认不了就如实说明还差什么，别让「不知道」被读成「没问题」。
    const protocolCheck = assessHttp2Precondition(cve, intel);
    if (protocolCheck.status === 'unknown') {
        limitations.push(`未能确认协议前置条件，需人工核查：${protocolCheck.detail}`);
    }
    ...
    return {
        ...
        intel: {
            ...
            tls: intel.tls ?? null,
            protocolHints: intel.protocolHints ?? null,
        },
        evidence,
        pentest,
        limitations,
...
/** 批量复测。 */
export async function retestBatch(runtime, targetUrl, cveIds, options = {}, signal) {
    ...
    for (const cveId of unique) {
        if (signal?.aborted)
            break;
        results.push(await retestOne(runtime, target, cveId, { ...options, saveReport: false, reproDoc: options.reproDoc ?? null }, signal));
    }
    const stats = {
    ...
    if (options.saveReport) {
        ...
        await writeFile(`${base}.json`, JSON.stringify({ ...batch, report: undefined }, null, 2), 'utf8');
...
/** 生成 Markdown 汇总报告。 */
export function renderBatchReport(batch) {
    const lines = [];
...
export function topPocLinks(result, limit = 3) {
