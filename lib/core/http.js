...
import { resolveTargetInput, swapScheme } from "./url.js";
...
export function baseHeaders(cfg, extra) {
    const headers = {
 ...
 */
export async function request(cfg, input, signal) {
    ...
    if (!PASSIVE_METHODS.has(method) && !cfg.allowNonIdempotentProbe) {
        throw new HttpRequestError('blocked-method', `${method} 属非幂等探测，需在配置中显式开启 allowNonIdempotentProbe`);
    }
    ...
    try {
        ...
        if (!retryable || !cfg.allowSchemeFallback || !PASSIVE_METHODS.has(method))
            throw error;
...
async function attemptRequest(cfg, url, method, input, signal, schemeFallback) {
    ...
    try {
        ...
        throw new HttpRequestError('network', `请求失败: ${error?.message ?? String(error)}`, error);
    }
    finally {
        clearTimeout(timer);
        if (signal)
 ...
 */
export async function fetchJson(cfg, url, headers, signal) {
    try {
        ...
        try {
            return { data: JSON.parse(response.bodyText), statusCode: response.statusCode };
        }
        catch (error) {
            return {
                data: null,
                statusCode: response.statusCode,
                error: `响应不是合法 JSON: ${error.message}`,
            };
        }
    }
    catch (error) {
