 ...
 */
const HTTP2_REQUIRED_CVES = new Set([
    // HTTP/2 Rapid Reset
    'CVE-2023-44487',
...
/** 描述里提到拒绝服务 / 资源耗尽语义。 */
const DOS_DESCRIPTION_PATTERN = /denial of service|resource exhaustion|memory exhaustion|cpu exhaustion|rapid reset|\bDoS\b|拒绝服务|资源耗尽/i;
/** 判断该 CVE 是否以「启用 HTTP/2」为触发前提。 */
export function requiresHttp2(cveInfo) {
 ...
 */
export function assessHttp2Precondition(cveInfo, intel) {
    const { required, basis } = requiresHttp2(cveInfo);
    if (!required)
        return { status: 'not-required', detail: '' };
 ...
 */
function applyProtocolPrecondition(cveInfo, intel, base) {
    const precondition = assessHttp2Precondition(cveInfo, intel);
    if (precondition.status === 'not-required')
        return base;
