...
const ALPN_OFFER = ['h2', 'http/1.1'];
/** 握手超时的下限：太短的超时会把正常的慢握手误判成「不支持」。 */
const TIMEOUT_FLOOR_MS = 3000;
/** 空结果，用于探测未执行或整体失败的场合。 */
export function emptyTlsIntel(error = null) {
    return {
        ok: false,
        error,
        protocol: null,
 ...
 * 不应该让整次指纹识别中断。
 */
export async function probeTls(options) {
...
function handshake(options) {
    return new Promise((resolve) => {
        ...
        try {
            ...
            finish(fail(`握手初始化失败: ${error.message}`));
            return;
        }
        if (options.signal) {
        ...
        socket.once('secureConnect', () => {
            try {
                ...
                const peer = current?.getPeerCertificate();
                ...
                finish(fail(`读取握手结果失败: ${error.message}`));
            }
            finally {
                socket?.end();
            }
...
function toCertificateIntel(peer) {
