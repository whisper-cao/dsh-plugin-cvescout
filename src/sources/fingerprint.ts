 ...
 *  - 每个探测动作都要过安全护栏，探测路径与是否做 OPTIONS/错误页探测都走配置。
 */
import type { TechComponent } from '../types.ts'
...
export async function fingerprintTarget(
  ...
  if (runtime.cfg.probeAllowedMethods) {
    try {
      ...
      result.probeErrors.push(`OPTIONS 探测失败: ${(error as Error).message}`)
    }
  }
  ...
  if (runtime.cfg.errorPageProbe) {
    try {
      ...
      result.probeErrors.push(`错误页探测失败: ${(error as Error).message}`)
    }
  }
  ...
  for (const path of runtime.cfg.passiveProbePaths) {
    try {
      ...
      result.probeErrors.push(`路径探测失败 ${path}: ${(error as Error).message}`)
    }
  }
