 ...
 * 原子能力工具。
 *
 * 每个工具做且只做一件事，由 Harness 的模型自行编排；所有调用统一过安全护栏
 * 并落审计。
 */
...
import { HttpRequestError, request } from '../core/http.ts'
import { intelFromFingerprint } from '../core/retest.ts'
import { fingerprintTarget } from '../sources/fingerprint.ts'
import { lookupCve } from '../sources/nvd.ts'
...
const TARGET_PARAM_DESCRIPTION =
  '目标地址：完整 URL、裸域名（portal.example.com）、host:port 均可；未写协议时按配置的 defaultScheme 补齐，连接失败会自动换协议重试一次；也接受 cordis.yml 中 targetAliases 配置的系统别名'

/** 安全护栏用的目标集合：已解析 URL + 原始输入（别名也要能被提示出来）。 */
function scopeParams(runtime: CvescoutRuntime, raw: string): Record<string, string> {
...
/** 注册全部原子工具。 */
export function registerAtomicTools(ctx: Context, runtime: CvescoutRuntime): void {
  // ---------------------------------------------------------------- CVE 查询
  ctx.tools.register(
    defineTool({
      name: 'cve_lookup',
      description:
        '查询 CVE 详情。优先查 NVD API 2.0，失败回退 GitHub Advisory。返回描述、CVSS 评分与等级、受影响组件及其版本区间、参考链接。用于复测前获取该 CVE 的适用条件。',
      parameters: {
  ...
  ctx.tools.register(
    defineTool({
      name: 'cve_poc_search',
      description:
        '搜索 CVE 相关的公开 PoC / 利用代码。来源为 GitHub 仓库搜索与 ExploitDB 检索页。ExploitDB 无公开 API，其结果是弱信号，需人工确认。',
      parameters: {
  ...
  ctx.tools.register(
    defineTool({
      name: 'http_probe',
      description:
        '向目标发送无害的 HTTP 探测请求，用于确认存活、响应头、错误页与 Allow 方法。受安全护栏约束：默认只允许 GET/HEAD/OPTIONS，禁止 TRACE/CONNECT，且目标必须命中授权域名白名单（拦截信息里会说明如何配置）。当用户给出具体 URL 并问「这个地址能不能访问 / 返回什么头 / 是否可达」时用本工具；问「某站点是否有某 CVE」请用 cve_retest。',
      parameters: {
      ...
      async execute(args, exec) {
        ...
        try {
          ...
          if (error instanceof HttpRequestError) throw new Error(`HTTP 探测失败（${error.kind}）: ${error.message}`)
          throw error
        }
      },
    }),
  ...
  ctx.tools.register(
    defineTool({
      name: 'fingerprint',
      description:
        '识别目标技术栈：服务器、框架、CMS、前端库、WAF、前置代理，并采集 Allow 方法、错误页特征与**协议能力**（TLS 版本、ALPN/HTTP-2 是否可用、证书事实）。全部为被动探测：HTTP 侧只用 GET/HEAD/OPTIONS 与固定被动路径，协议侧只做一次裸 TLS 握手、不发任何载荷，不做任何写入。当需要「先摸清某个站点用什么组件、是否开着 HTTP/2」而不针对特定 CVE 时用本工具。',
      parameters: {
        url: { type: 'string', required: true, description: TARGET_PARAM_DESCRIPTION },
  ...
  ctx.tools.register(
    defineTool({
      name: 'intel_cache',
      description:
        '管理目标情报缓存（JSON 文件 + TTL）。action=get 读单个目标已缓存的情报；list 列出全部条目；invalidate 删除单个目标；clear 清空。用户问「上次扫描结果是什么 / 缓存了哪些目标 / 清一下缓存」时用本工具。',
      parameters: {
        action: {
          type: 'string',
      ...
      async execute(args, exec) {
        ...
        switch (args.action) {
          case 'get': {
            if (!resolvedUrl) throw new Error('action=get 需要提供 url')
          ...
          case 'invalidate': {
            if (!resolvedUrl) throw new Error('action=invalidate 需要提供 url')
  ...
  ctx.tools.register(
    defineTool({
      name: 'target_scope',
      description:
        '查看本插件的授权目标范围与配置状态：授权域名白名单、系统别名映射（例如「内部管理系统」→ 具体域名）、默认协议、非幂等探测是否开启、速率与总量配额、情报缓存与报告路径。' +
  ...
  ctx.tools.register(
    defineTool({
      name: 'safety_audit',
      description:
        '读取本插件运行期的安全审计：工具调用总数、拦截次数与原因、当前速率与总量配额用量、授权域名白名单。参数已做敏感字段脱敏。',
      parameters: {
